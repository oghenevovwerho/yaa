...
import static yaa.pojos.GlobalData.double$name;

public class IntMold {
  public static YaaClz newInt() {
    var clz = new YaaClz(int$name);
    var lsm = lShiftMtds();
    clz.instanceMethods.put(lsm.name, lsm);
    var rsm = rShiftMtds();
    clz.instanceMethods.put(rsm.name, rsm);
    var r_usm = uRShiftMtds();
    clz.instanceMethods.put(r_usm.name, r_usm);
    ...
    var xor$methods = xorMtds();
    clz.instanceMethods.put(xor$methods.name, xor$methods);
    var bitOr$methods = bitOrMtds();
    clz.instanceMethods.put(bitOr$methods.name, bitOr$methods);
    var bitAnd$methods = bitAndMtds();
    clz.instanceMethods.put(bitAnd$methods.name, bitAnd$methods);
    var bitNegate$methods = bitNegate();
    clz.instanceMethods.put(bitNegate$methods.name, bitNegate$methods);
    var toStringMtd = MoldUtils.toStringMtds(int$name);
    clz.instanceMethods.put("toString", toStringMtd);
    var hashCodeMtd = MoldUtils.hashCodeMtds(int$name);
    clz.instanceMethods.put("hashCode", hashCodeMtd);
    var getClassMtd = MoldUtils.getClassMtds(int$name);
    clz.instanceMethods.put("getClass", getClassMtd);

    var box$fun = new YaaFun("box");
    box$fun.type = new YaaClz("java.lang.Integer");
  ...
  private static MtdPack unaryPlus() {
    ...
    mtd$list.type = new YaaClz(int$name);
    var unary$mtd$list = new LinkedList<YaaFun>();
    unary$mtd$list.add(mtd$list);
    return new MtdPack(unary$mtd$list, unary_plus_op_name);
  }

  private static MtdPack bitNegate() {
    var mtd$list = new$fun(bit_negate_op_name);
    mtd$list.type = new YaaClz(int$name);
    var unary$mtd$list = new LinkedList<YaaFun>();
    unary$mtd$list.add(mtd$list);
    return new MtdPack(unary$mtd$list, bit_negate_op_name);
  }
  ...
  private static MtdPack xorMtds() {
    var mtd$list = new LinkedList<YaaFun>();

    var right$int = new$fun(xor_op_name);
    right$int.parameters.add(new YaaClz(int$name));
    right$int.type = new YaaClz(int$name);
    mtd$list.add(right$int);

    var right$long = new$fun(xor_op_name);
    right$long.parameters.add(new YaaClz(long$name));
    right$long.type = new YaaClz(int$name);
    mtd$list.add(right$long);

    return new MtdPack(mtd$list, xor_op_name);
  }

  private static MtdPack bitOrMtds() {
    var mtd$list = new LinkedList<YaaFun>();

    var right$int = new$fun(bit_or_op_name);
    right$int.parameters.add(new YaaClz(int$name));
    right$int.type = new YaaClz(int$name);
    mtd$list.add(right$int);

    var right$long = new$fun(bit_or_op_name);
    right$long.parameters.add(new YaaClz(long$name));
    right$long.type = new YaaClz(int$name);
    mtd$list.add(right$long);

    return new MtdPack(mtd$list, bit_or_op_name);
  }

  private static MtdPack bitAndMtds() {
    var mtd$list = new LinkedList<YaaFun>();

    var right$int = new$fun(bit_and_op_name);
    right$int.parameters.add(new YaaClz(int$name));
    right$int.type = new YaaClz(int$name);
    mtd$list.add(right$int);

    var right$long = new$fun(bit_and_op_name);
    right$long.parameters.add(new YaaClz(long$name));
    right$long.type = new YaaClz(int$name);
    mtd$list.add(right$long);

    return new MtdPack(mtd$list, bit_and_op_name);
  }

  private static MtdPack uRShiftMtds() {
    var mtd$list = new LinkedList<YaaFun>();

    var right$int = new$fun(ur_shift_op_name);
    right$int.parameters.add(new YaaClz(int$name));
    right$int.type = new YaaClz(int$name);
    mtd$list.add(right$int);

    var right$long = new$fun(ur_shift_op_name);
    right$long.parameters.add(new YaaClz(long$name));
    right$long.type = new YaaClz(int$name);
    mtd$list.add(right$long);

    var right$Short = new$fun(ur_shift_op_name);
    right$Short.parameters.add(new YaaClz(short$name));
    right$Short.type = new YaaClz(int$name);
    mtd$list.add(right$Short);

    var right$Byte = new$fun(ur_shift_op_name);
    right$Byte.parameters.add(new YaaClz(byte$name));
    right$Byte.type = new YaaClz(int$name);
    mtd$list.add(right$Byte);

    return new MtdPack(mtd$list, ur_shift_op_name);
  }

  private static MtdPack lShiftMtds() {
    var mtd$list = new LinkedList<YaaFun>();

    var right$int = new$fun(left_shift_op_name);
    right$int.parameters.add(new YaaClz(int$name));
    right$int.type = new YaaClz(int$name);
    mtd$list.add(right$int);

    var right$long = new$fun(left_shift_op_name);
    right$long.parameters.add(new YaaClz(long$name));
    right$long.type = new YaaClz(int$name);
    mtd$list.add(right$long);

    var right$Short = new$fun(left_shift_op_name);
    right$Short.parameters.add(new YaaClz(short$name));
    right$Short.type = new YaaClz(int$name);
    mtd$list.add(right$Short);

    var right$Byte = new$fun(left_shift_op_name);
    right$Byte.parameters.add(new YaaClz(byte$name));
    right$Byte.type = new YaaClz(int$name);
    mtd$list.add(right$Byte);

    return new MtdPack(mtd$list, left_shift_op_name);
  }

  private static MtdPack rShiftMtds() {
    var mtd$list = new LinkedList<YaaFun>();

    var right$int = new$fun(right_shift_op_name);
    right$int.parameters.add(new YaaClz(int$name));
    right$int.type = new YaaClz(int$name);
    mtd$list.add(right$int);

    var right$long = new$fun(right_shift_op_name);
    right$long.parameters.add(new YaaClz(long$name));
    right$long.type = new YaaClz(int$name);
    mtd$list.add(right$long);

    var right$Short = new$fun(right_shift_op_name);
    right$Short.parameters.add(new YaaClz(short$name));
    right$Short.type = new YaaClz(int$name);
    mtd$list.add(right$Short);

    var right$Byte = new$fun(right_shift_op_name);
    right$Byte.parameters.add(new YaaClz(byte$name));
    right$Byte.type = new YaaClz(int$name);
    mtd$list.add(right$Byte);

    return new MtdPack(mtd$list, right_shift_op_name);
  }
  ...
  private static MtdPack rootMtds() {
    ...
    right$Short.parameters.add(new YaaClz(short$name));
    right$Short.type = new YaaClz(double$name);
    mtd$list.add(right$Short);

    var right$Byte = new$fun(root_op_name);
    right$Byte.parameters.add(new YaaClz(byte$name));
    right$Byte.type = new YaaClz(double$name);
    mtd$list.add(right$Byte);
  ...
  private static MtdPack divideMtds() {
    ...
    right$long.parameters.add(new YaaClz(long$name));
    right$long.type = new YaaClz(long$name);
    mtd$list.add(right$long);
  ...
  private static MtdPack plusMtds() {
    ...
    var right$char = new$fun(plus_op_name);
    right$char.parameters.add(new YaaClz(char$name));
    right$char.type = new YaaClz(int$name);
  ...
  private static YaaFun new$fun(String mtd$name) {
    return new YaaFun(mtd$name, int$name);
  }
}